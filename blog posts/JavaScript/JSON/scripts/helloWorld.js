...
function init(){
    ...
    var obj = JSON.parse('{"name":"John","last":"Resig"}', function( key, value ) {
    ...
    var st = new $jit.ST({
        ...
        //This method gets called when a node is clicked and its subtree has a smaller depth than the one specified by the levelsToShow parameter.
        //In that case a subtree is requested and is added to the dataset.
        //This method is asynchronous, so you can make an Ajax request for that subtree and then handle it to the onComplete callback.
        ...
        //This method is called right before plotting a node. It's useful for changing an individual node style properties before plotting it.
        //The data properties prefixed with a dollar sign will override the global node style properties.
        onBeforePlotNode: function(node){
            //add some color to the nodes in the path between the root node and the selected node.
            if (node.selected) {
                node.data.$color = "#ff7";
            }
            else {
